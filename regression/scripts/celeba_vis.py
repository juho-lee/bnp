...
if __name__ == '__main__':
    ...
    # load anp
    with open('configs/celeba/anp.yaml', 'r') as f:
        config = yaml.safe_load(f)
    anp = ANP(**config).cuda()
    ckpt = torch.load(osp.join(results_path, 'celeba', 'anp', args.expid, 'ckpt.tar'))
    anp.load_state_dict(ckpt['model'])

    # load banp
    with open('configs/celeba/banp.yaml', 'r') as f:
        config = yaml.safe_load(f)
    banp = BANP(**config).cuda()
    ckpt = torch.load(osp.join(results_path, 'celeba', 'banp', args.expid, 'ckpt.tar'))
    banp.load_state_dict(ckpt['model'])

    torch.manual_seed(args.seed)
    torch.cuda.manual_seed(args.seed)

    loader = torch.utils.data.DataLoader(CelebA(train=False),
    ...
    orig_I = coord_to_img(batch.x, batch.y+0.5, (3, 32, 32)).clamp(0,1)
    ctx_I = coord_to_img(batch.xc, batch.yc+0.5, (3, 32, 32)).clamp(0,1)
    ...
    bsigma = bvar.sqrt()
    anp_I = coord_to_img(batch.x, (bmu+0.5).clamp(0,1), (3, 32, 32)).clamp(0,1)
    anp_std = coord_to_img(batch.x, bsigma, (3, 32, 32)).clamp(0,1)
    ...
    bvar = sigma.pow(2).mean(0) + mu.pow(2).mean(0) - mu.mean(0).pow(2)
    bsigma = bvar.sqrt()
    banp_I = coord_to_img(batch.x, (bmu+0.5).clamp(0,1), (3, 32, 32)).clamp(0,1)
    banp_std = coord_to_img(batch.x, bsigma, (3, 32, 32)).clamp(0,1)
