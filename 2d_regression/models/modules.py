...
class Encoder(nn.Module):
    def __init__(self, dim_x=2, dim_y=1, dim_hid=128,
        ...
        if self_attn:
            ...
            self.self_attn = MultiHeadAttention(
                    dim_hid, dim_hid, dim_hid, dim_hid)
        else:
            self.mlp_pre = nn.Sequential(
                    nn.Linear(dim_x+dim_y, dim_hid), nn.ReLU(True),
                    nn.Linear(dim_hid, dim_hid), nn.ReLU(True),
    ...
    def forward(self, x, y):
        ...
        if self.self_attn:
            hid = self.self_attn(hid, hid, hid)
        if self.use_latent:
...
class Decoder(nn.Module):
    def __init__(self, dim_x=2, dim_y=1, dim_enc=128, dim_hid=128):
        super().__init__()
...
class AttEncoder(nn.Module):
    def __init__(self, dim_x=1, dim_y=1, dim_hid=128,
        ...
        if self_attn:
            self.mlp_v = nn.Sequential(
                    ...
                    nn.Linear(dim_hid, dim_hid), nn.ReLU(True),
                    nn.Linear(dim_hid, dim_hid))
            self.self_attn = MultiHeadAttention(dim_hid, dim_hid, dim_hid, dim_hid)
        else:
            self.mlp_v = nn.Sequential(
                    nn.Linear(dim_x+dim_y, dim_hid), nn.ReLU(True),
                    nn.Linear(dim_hid, dim_hid), nn.ReLU(True),
        ...
        self.mlp_qk = nn.Sequential(
                ...
                nn.Linear(dim_hid, dim_hid), nn.ReLU(True),
                nn.Linear(dim_hid, dim_hid))
        self.cross_attn = MultiHeadAttention(dim_hid, dim_hid, dim_hid,
    ...
    def forward(self, xc, yc, xt):
        ...
        if self.use_latent:
            mu, sigma = self.cross_attn(query, key, value).chunk(2, -1)
            sigma = 0.1 + 0.9 * torch.sigmoid(sigma)
            return Normal(mu, sigma)
        else:
            return self.cross_attn(query, key, value)
